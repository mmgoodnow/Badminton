...
final class PlexAccountListViewModel: ObservableObject {
    ...
    private let accountResolver = PlexAccountResolver.shared
    ...
    func load(token: String?, preferredServerID: String?, force: Bool = false) async {
        ...
        do {
            ...
            let accountMapping = await accountResolver.mapHomeUsersToAccountIDs(
                homeUsers: Array(homeUsers.values),
                candidateAccountIDs: Set(counts.keys),
                token: token
            )
            accounts = homeUsers.values
                .map { user in
                    let matchedAccountIDs = accountMapping[user.id] ?? []
